...
pub const PresentCircumstances = struct {
	...
	pub fn fromBytes(allocator: std.mem.Allocator, offset: usize, buffer: []const u8) !struct { value: PresentCircumstances, bytes_read: usize } {
		...
		const PresentCircumstances_visibleEntities_read = try readList(Entity, allocator, local_offset, buffer);
		const PresentCircumstances_visibleEntities = PresentCircumstances_visibleEntities_read.value;
		local_offset += PresentCircumstances_visibleEntities_read.bytes_read;
		...
		return .{ .value = PresentCircumstances{
			...
			.visibleEntities = PresentCircumstances_visibleEntities,
		}, .bytes_read = local_offset - offset };
	}
	...
	pub fn writeBytes(self: *const PresentCircumstances, offset: usize, buffer: []u8, tag: bool) usize {
		...
		local_offset += writeList(Entity, local_offset, buffer, self.visibleEntities);
	...
	pub fn deinit(self: *PresentCircumstances, allocator: std.mem.Allocator) void {
		...
		for (self.visibleEntities) |*item2| {
			item2.deinit(allocator);
		}
		allocator.free(self.visibleEntities);
	}
};
