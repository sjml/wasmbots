...
typedef struct {
	...
	uint16_t visibleEntities_len;
	WasmBots_Entity* visibleEntities;
...
WasmBots_err_t WasmBots_PresentCircumstances_GetSizeInBytes(const WasmBots_PresentCircumstances* m, size_t* size) {
	...
	*size += m->surroundings_len * 1;
	for (uint16_t i1 = 0; i1 < m->visibleEntities_len; i1++) {
		*size += m->visibleEntities[i1].label_len;
		*size += 18;
	}
	*size += 12;
	return WASMBOTS_ERR_OK;
...
WasmBots_PresentCircumstances* WasmBots_PresentCircumstances_Create(void) {
	...
	out->visibleEntities = WasmBots_PresentCircumstances_default.visibleEntities;
	return out;
}
...
void WasmBots_PresentCircumstances_Cleanup(WasmBots_PresentCircumstances *m) {
	...
	for (uint16_t i1 = 0; i1 < m->visibleEntities_len; i1++) {
		WASMBOTS_FREE(m->visibleEntities[i1].label);
	}
	WASMBOTS_FREE(m->visibleEntities);
}
...
WasmBots_err_t WasmBots_PresentCircumstances_FromBytes(WasmBots_DataAccess* r, WasmBots_PresentCircumstances* dst) {
	...
	err = WasmBots__ReadUInt16(r, &(dst->visibleEntities_len));
	if (err != WASMBOTS_ERR_OK) {
		return err;
	}
	dst->visibleEntities = (WasmBots_Entity*)WASMBOTS_MALLOC(sizeof(WasmBots_Entity) * dst->visibleEntities_len);
	if (dst->visibleEntities == NULL) { return WASMBOTS_ERR_ALLOCATION_FAILURE; }
	for (uint16_t i1 = 0; i1 < dst->visibleEntities_len; i1++) {
		err = WasmBots_Entity_FromBytes(r, &(dst->visibleEntities[i1]));
		if (err != WASMBOTS_ERR_OK) {
			return err;
		}
	}
...
WasmBots_err_t WasmBots_PresentCircumstances_WriteBytes(WasmBots_DataAccess* w, const WasmBots_PresentCircumstances* src, bool tag) {
	...
	err = WasmBots__WriteUInt16(w, (src->visibleEntities_len));
	if (err != WASMBOTS_ERR_OK) {
		return err;
	}
	for (uint16_t i1 = 0; i1 < src->visibleEntities_len; i1++) {
		err = WasmBots_Entity_WriteBytes(w, &(src->visibleEntities[i1]));
		if (err != WASMBOTS_ERR_OK) {
			return err;
		}
	}
