...
export class PresentCircumstances extends Message {
	...
	getSizeInBytes(): usize {
		...
		for (let visibleEntities_i=0; visibleEntities_i < this.visibleEntities.length; visibleEntities_i++) {
			size += String.UTF8.encode(this.visibleEntities[visibleEntities_i].label, false).byteLength;
			size += 18;
		}
		size += 12;
		return size;
	...
	static override fromBytesDA(da: DataAccess): PresentCircumstances | null {
		...
		for (let i2: u16 = 0; i2 < visibleEntities_Length; i2++) {
			const _visibleEntities_i2_ = Entity.fromBytes(da);
			if (_visibleEntities_i2_ == null) {
				return null;
			}
			else {
				nPresentCircumstances.visibleEntities[i2] = _visibleEntities_i2_;
			}
		}
	...
	writeBytesDA(da: DataAccess, tag: boolean): bool {
		...
		if (da.hasError) { return false; }
		da.setUint16(this.visibleEntities.length as u16);
		if (da.hasError) { return false; }
